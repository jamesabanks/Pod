...
int scan_blurb_pre_size(pod_stream *stream, pod_char_t c)
{
    int warning;
...
int scan_blurb_size(pod_stream *stream, pod_char_t c)
{
    ...
    switch (c) {
        ...
        case ' ':
            ...
            stream->state &= pod_state_mask;
            stream->state |= pod_blurb_post_size;
            break;
        case '':
            // unexpected end
            pod_stream_add_token(stream, pod_token_pod_sync);
...
int scan_blurb_post_size(pod_stream *stream, pod_char_t c)
{
    int warning;
